...
function assertCommercialLicense(value) {
    if (typeof value !== "object" ||
        value === null ||
        Array.isArray(value)) {
        throw new EGALicenseKeyError("EGA_LICENSE_KEY_PAYLOAD", "Commercial License payload must be an object.");
    }
    const license = value;
    if (license.schemaVersion !== 1 ||
        license.licenseKind !==
            "commercial") {
        throw new EGALicenseKeyError("EGA_LICENSE_KEY_TYPE", "Commercial License Key must contain a version 1 commercial license.");
    }
    const requiredStringFields = [
        "licenseId",
        "contactName",
    ...
    for (const field of requiredStringFields) {
        if (typeof license[field] !==
            "string" ||
            license[field].trim().length ===
                0) {
            throw new EGALicenseKeyError("EGA_LICENSE_KEY_PAYLOAD", `Commercial License payload requires a non-empty ${field}.`);
        }
    }
    const email = String(license.workEmail);
    if (!email.includes("@") ||
        email.startsWith("@") ||
        email.endsWith("@")) {
        throw new EGALicenseKeyError("EGA_LICENSE_KEY_PAYLOAD", "Commercial License payload contains an invalid workEmail.");
    }
    const issuedAt = new Date(String(license.issuedAt));
    if (Number.isNaN(issuedAt.getTime())) {
        throw new EGALicenseKeyError("EGA_LICENSE_KEY_PAYLOAD", "Commercial License issuedAt must be a valid ISO-8601 date string.");
    }
    if (license.expiresAt !==
        undefined) {
        if (typeof license.expiresAt !==
            "string" ||
            license.expiresAt.trim()
                .length === 0) {
            throw new EGALicenseKeyError("EGA_LICENSE_KEY_PAYLOAD", "Commercial License expiresAt must be a valid non-empty ISO-8601 date string when provided.");
        }
        const expiresAt = new Date(license.expiresAt);
        if (Number.isNaN(expiresAt.getTime()) ||
            expiresAt.getTime() <=
                issuedAt.getTime()) {
            throw new EGALicenseKeyError("EGA_LICENSE_KEY_PAYLOAD", "Commercial License expiresAt must be later than issuedAt.");
        }
    }
}
function assertLicense(value) {
    if (typeof value !== "object" ||
        value === null ||
        Array.isArray(value)) {
        throw new EGALicenseKeyError("EGA_LICENSE_KEY_PAYLOAD", "License payload must be an object.");
    }
    const license = value;
    if (license.licenseKind ===
        "evaluation") {
    ...
    throw new EGALicenseKeyError("EGA_LICENSE_KEY_TYPE", "License Key contains an unsupported license type.");
}
/**
...
function verifyLicenseKey(licenseKey, publicKey) {
    if (typeof licenseKey !== "string" ||
        ...
        throw new EGALicenseKeyError("EGA_LICENSE_KEY_FORMAT", "EGA V9 License Key is required.");
    }
    const parts = licenseKey.split(".");
    if (parts.length !== 3 ||
        parts[0] !== LICENSE_KEY_PREFIX) {
        throw new EGALicenseKeyError("EGA_LICENSE_KEY_FORMAT", "EGA V9 License Key has an unsupported format.");
    }
    const payloadBuffer = decodeBase64Url(parts[1]);
    const signatureBuffer = decodeBase64Url(parts[2]);
    const signatureIsValid = (0, crypto_1.verify)(null, payloadBuffer, toPublicKey(publicKey), signatureBuffer);
    if (!signatureIsValid) {
        throw new EGALicenseKeyError("EGA_LICENSE_KEY_SIGNATURE", "EGA V9 License Key signature verification failed.");
    }
    let parsed;
    try {
        parsed =
        ...
        throw new EGALicenseKeyError("EGA_LICENSE_KEY_PAYLOAD", "EGA V9 License Key payload is not valid JSON.");
    }
    assertLicense(parsed);
    const canonicalPayload = serializeLicenseForSigning(parsed);
    if (canonicalPayload !==
        payloadBuffer.toString("utf8")) {
        throw new EGALicenseKeyError("EGA_LICENSE_KEY_PAYLOAD", "EGA V9 License Key payload is not canonical.");
    }
    return parsed;
}
